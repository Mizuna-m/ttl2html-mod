...
module TTL2HTML
  class Template
    ...
    def expand_shape(data, uri, prefixes = {})
      return nil unless data[uri]
      return nil unless data[uri]['http://www.w3.org/ns/shacl#property']
      ...
      result = data[uri]['http://www.w3.org/ns/shacl#property'].sort_by do |e|
        ...
        if data[property]['http://www.w3.org/ns/shacl#maxCount']
          ...
          repeatable = true if max_count > 1
        else
          repeatable = true
        end
        nodes = nil
    ...
    def get_language_literal(object)
      if object.is_a? Array
        ...
        # end
        object_lang = object.select do |e|
          e.is_a?(RDF::Literal) && e.language? && e.language == I18n.locale
        end
        if !object_lang.empty?
          object_lang.first
        else
          object.first
        end
    ...
    def format_property(property, labels = {}, subject = nil)
      subject = @param[:blank_subject] if !subject and @param[:blank_subject]
      ...
      if subject_class and @param[:labels_with_class][subject_class] and @param[:labels_with_class][subject_class][property]
        ...
        property.split(%r{[/\#]}).last.capitalize
      end
    end

    def format_object(object, _data, type = {})
      if %r{\Ahttps?://} =~ object.to_s
